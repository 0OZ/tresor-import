...
const getDocumentType = content => {
    if (content.includes("Wertpapierabrechnung: Kauf")) {
        ...
        return ActivityType.ACCOUNT_STATEMENT;
    } else if (content.includes("Rechnungsabschluss: EUR")) {
        return ActivityType.ACCOUNT_CLEARING;
    }
};
...
const findShares = (content, documentType) => {
    let line;
    if (documentType === ActivityType.DIVIDEND) {
        line = content[findLineNumberByContent(content, 'Ausschüttung', false) - 1];
    } else if ([ActivityType.BUY, ActivityType.SELL].includes(documentType)) {
        line =
            content[
                findLineNumberByCurrentAndPreviousLineContent(
                    content,
...
const findAmount = (content, documentType) => {
    ...
    } else if (documentType === ActivityType.BUY || documentType === ActivityType.SELL) {
...
export const parsePages = contents => {
    ...
    const inlineContent = content.join('\n')
    const documentType = getDocumentType(inlineContent);

    switch (documentType) {
        case ActivityType.ACCOUNT_CLEARING:
            return {
                activities,
                status: 7,
            };

        case ActivityType.ACCOUNT_STATEMENT:
